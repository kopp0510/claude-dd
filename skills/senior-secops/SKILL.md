...
- **實作方式**：透過 `Task(subagent_type="senior-secops:senior-secops" 或 "senior-secops", ...)` 調用官方 agent（優先 plugin 命名空間，失敗 fallback 本地 `~/.claude/agents/senior-secops.md`）
...
**Agent 調用策略**：先試 plugin 命名空間 `subagent_type="senior-secops:senior-secops"`，若收到 `Agent type '...' not found` 錯誤則 fallback 短名稱 `subagent_type="senior-secops"`（從 `~/.claude/agents/senior-secops.md` 讀取，由 `install-dd-pipeline.sh` 部署）。兩者皆失敗時停下，告訴使用者執行 `./install-dd-pipeline.sh` 補齊本地 agent。
