...
清單交出去前先過下方 Taste Gate 的「**結構清單**」組 —— 數量上限（節點 ≤9、連線 ≤12、
分組 ≤4）在這一步就數得出來，此時超標只要改清單；等到第 4 步才發現，只能整份重來。
...
- 量化預算三組，**數字一律以 `references/composition-quality-contract.md` 為準**，
  下列只是最常違反的提醒。超出 → 調節點位置，或拆成總覽 + 細節兩張；不縮字級、不壓間距硬塞：
  - **版面**：0 交叉、每邊 ≤2 折、繞路比 ≤1.35、節點間 ≥80px、容器 gutter ≥20px
  - **數量**（「元素數量預算」）：節點 ≤9、連線 ≤12、強調色 ≤2、分組 ≤4、註解框 ≤2
  - **連線**（「連線可量測規則」）：邊標籤遮罩與線留 6–10px 可見間隙、同側多線分 port
    （相鄰 ≥12px，小節點最低 8px）、遮罩不可被之後才畫的節點蓋住、連線不穿過非端點節點
    （不可避免時改虛線並把標籤移到可見端）
- **節點間距取嚴為 80px**（`svg-layout-best-practices` 的 Universal Layout Rules）。
  contract 表列的 40px 是上游 showcase profile 的不及格線，本 skill 不用它當閘門。
  標籤有三個容易混淆的數字，量的不是同一件事：遮罩與**自己那條線**的可見間隙 6–10px、
  標籤相對連線的垂直偏移 5–10px、邊標籤與**無關幾何**的淨空 ≥4px
