...
- 量化版面預算照 `references/composition-quality-contract.md`：0 交叉、每邊 ≤2 折、
  ...
  同側多線分 port（相鄰 ≥12px）、legend 不進流程走廊。超出預算 → 調節點位置或拆圖，不硬塞
- **元素數量上限**（同檔「元素數量預算」）：節點 ≤9、連線 ≤12、強調色 ≤2、
  分組 ≤4、註解框 ≤2。超過即拆成總覽 + 細節兩張，不縮字級硬塞
- 邊標籤遮罩與線之間留 6–10px 可見間隙；遮罩不可被之後才畫的節點蓋住；
  連線不穿過非端點節點（不可避免時改虛線並把標籤移到可見端）— 細則同檔「連線可量測規則」
...
檢查項一律走下方「**產出前檢查清單（Taste Gate）**」逐項打勾，不憑印象。
...
第 3 步寫完 SVG、進第 4 步渲染自檢時逐項打勾。這是把散在各步的檢查項收攏成一張表，
不是新規則；每項後面括號指回本文出處。**任一項不過 → 回去改 SVG，不進 GIF 匯出。**

**該不該畫**

- [ ] 這張圖比一段文字或一張表格帶給讀者更多？（硬閘門 1）
- [ ] 結構清單已被使用者確認或修改定案？（硬閘門 2 / 第 1 步）

**刪減測試**（借鏡 diagram-design，MIT）

- [ ] 有沒有哪個節點可以刪掉，讀者仍看得懂？
- [ ] 有沒有兩個節點總是一起出現，該併成一個？
- [ ] 有沒有哪條連線是版面已經講清楚的，可以刪？
- [ ] 有沒有哪個標籤是顏色或形狀已經表達過的，可以刪？

**數量與版面**

- [ ] 節點 ≤9、連線 ≤12、強調色元素 ≤2？超過即拆圖（composition-quality-contract「元素數量預算」）
- [ ] 0 交叉、每邊 ≤2 折、繞路比 ≤1.35、節點間 ≥40px、容器 gutter ≥20px？
- [ ] 邊標籤與其連線之間留 6–10px **可見**間隙，遮罩沒有壓到線？
- [ ] 同一邊多條連線各有自己的 port，相鄰 ≥12px？
- [ ] 標籤遮罩沒有被之後才畫的節點蓋掉？
- [ ] 連線沒有穿過非端點的節點（不可避免時改虛線，標籤移到可見端）？
- [ ] 座標對齊 8px 網格？（svg-layout-best-practices）
- [ ] legend 在流程走廊外、文字沒有溢出節點邊界？

**動畫與 GIF**

- [ ] 所有 dur 與 begin 整除同一總循環長？（第 3 步）
- [ ] 錯開相位用負值 `begin`，沒有球停在 (0,0)？
- [ ] 定格兩個時間點截圖比對過，小球確實有位移？（第 4 步）

**交付**

- [ ] SVG 零 script 檢查通過、暫存幀與中間產物已清理？（第 6 步）
- [ ] 成品在 `./diagrams/`，原尺寸未壓縮？（第 6 步）
