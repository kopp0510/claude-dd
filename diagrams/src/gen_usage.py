...
ZH = dict(
    ...
    boxes=[
        ...
        ("④ /dd-init", "蓋章 7 步迴圈到專案 CLAUDE.md",
         "掛 pre-commit gate · 建 .screenshots/ · 檢查 plugin 依賴", VIOLET),
        ("⑤ 7 步開發迴圈", "實作+測試 → commit → 簡化",
         "→ 審查 → 再測（curl/playwright）→ commit → 沉澱", GREEN),
...
EN = dict(
    ...
    boxes=[
        ...
        ("④ /dd-init", "stamps the 7-step loop into CLAUDE.md",
         "hooks the gate · creates .screenshots/ · checks plugins", VIOLET),
        ("⑤ 7-step development loop", "implement + test → commit → simplify",
         "→ review → re-verify (curl/playwright) → commit → capture", GREEN),
