...
- **task-planner 改成功能段落規劃，進度表寫進設計文件**。原本拆的是「2–5 分鐘的微任務」，
  ...
  計畫檔也被刪掉了。改成兩層：段落（一圈 8 步，驗收寫「步驟 5 要看到什麼」，碰到錢、權限、
  刪資料的單獨成段）與段落內的小任務（`S2-1` 編號，實作、驗證、commit，commit 訊息帶編號）；
  小任務 commit 了不等於段落完成，整段跑完步驟 3–8 才標 `DONE`。進度表寫進設計文件（沒有
  設計文件就在 `docs/designs/` 開一份短的），先列出要新增的測試、使用者批准後才寫檔並 commit，
  下一段固定挑表上第一個依賴已完成的 `TODO`。全域模板 §4.4 的檔案式追蹤跟著改用這張表
  （取代下面那條的 `docs/plans/` 與 🚧 / ✅），design-brainstorm 的交接說明同步
