...
- **巢狀檔撐不過 `/compact`。** 官方寫得很明確：專案根目錄的 `CLAUDE.md` 在壓縮後會重新注入，
  但「子目錄的巢狀 CLAUDE.md 與帶 `paths:` frontmatter 的 rules 不會自動重新注入；它們要等下次 Claude 讀到該子目錄的檔案時才重新載入」。
  長 session 裡，你正在倚賴的那條子系統規則可能已經悄悄離開 context 了。
- **檔案越多，矛盾越多。** 官方原文：「兩條規則互相牴觸時，Claude 可能任選一條」。巢狀會放大這個表面積，
  而官方給的解法是人工的：定期檢視巢狀檔案、刪掉衝突。
- **小改動的真實摩擦。** 動了某個沒有 `CLAUDE.md` 的目錄裡的一支 `.sh`，commit 就被擋到你寫出一份為止。
  有時這正是重點；有時你只是想修腳本裡的一個錯字，`SKIP_DOC_CHECK=1` 就是那個洩壓閥 —
  當然這也表示這套紀律的強度，取決於你有多不想去按那個逃生口。
- **文件形式主義是真實風險。** 沒有任何東西檢查你寫的 `CLAUDE.md` **內容是否正確** — 只檢查它存在、且有被 staged。
  一個塞滿沒人讀的檔案的目錄，可以完美通過 gate。

如果這些代價聽起來比你手上的問題更糟，那就用單一根目錄 `CLAUDE.md`、不要裝 gate。6 步迴圈沒有它一樣能跑。
