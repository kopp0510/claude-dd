...
安裝程式會：
1. 安裝 53 個內建 Skills 到 `~/.claude/skills/`（自動觸發專家知識）
...
| 指令 | 說明 |
|------|------|
| `/dd-init` | 初始化專案，建立 `claude_docs/` 目錄與專案設定（支援自動偵測現有專案，Phase 4 預設呼叫 DX Engineer） |
...
用戶輸入需求
      ...
      ▼
   /dd-init ─────────► 初始化專案結構
      │                 └─ Phase 4 (opt-in): 呼叫 dx-engineer 設計 golden path
      │                    (可日後跑 /dd-dx 補)
      ▼
   /dd-start ────────► 需求分析 (REQUIREMENTS.md)
      │
      ▼
   /dd-arch ─────────► 架構設計（SDD/DDD/ADD/EDD/UI 設計全部寫入 plan file）
      │
      ▼
┌─────────────────────────────────────┐
│  🔒 人工審核 Checkpoint              │
│  /dd-approve 批准                   │
│  （需修改時：直接編輯 plan file 後   │
│   重跑 /dd-approve）                 │
│  → 產生 REQUIREMENTS.md +           │
│    API_CONTRACT.md                  │
└──────────┬──────────────────────────┘
...
現有專案（已有程式碼）
   ...
   /dd-init ─────────► 自動偵測技術棧，補充 DD 設定
      │                 └─ Phase 4 (opt-in): dx-engineer 產出 golden path
      ▼
   /docs-gen ────────► 分析程式碼，產生跨界面契約文檔
...
> **2026-05 流程改版**：設計類內容(SDD/DDD/ADD/EDD/UI 設計)留在 `plans/<feature>.md` 內,不再轉檔到獨立檔案。`claude_docs/` 只持久化跨界面契約與最終報告。

DD Pipeline 會在專案中建立 `claude_docs/` 目錄，包含：

- `CLAUDE.md` - 專案設定與規範
- `PROJECT_STATE.md` - 專案狀態追蹤
- `requirements/REQUIREMENTS.md` - 需求規格(RDD)
- `contracts/API_CONTRACT.md` - API 契約(DbC)
- `plans/<feature>.md` - 完整設計(SDD/DDD/ADD/EDD/UI)+ 微任務計畫(SADD)
- `reports/TEST_REPORT.md`、`RELEASE_NOTES.md` - 測試與發布報告
...
透過包裝外部命令或工作流程，提供關鍵詞自動觸發能力。

> **2026-05 精簡**：原本 10 個「純 Agent 包裝器」skill（senior-frontend/backend/devops/fullstack/qa/secops、tdd-guide、playwright-pro、claude-api-expert、code-reviewer）已移除。對應的 Agent 仍存在於 `~/.claude/agents/`，由 Claude 透過 §7.2 主動觸發機制 + agent description 比對直接派 Task，不再經過 wrapper skill 中轉。
