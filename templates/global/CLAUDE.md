...
**有進度表時 `--segment-base` 不是最終答案**:專案有 `task-planner` 產的進度表
(`docs/designs/…` 的「進度表」)時,`<起點>` 以**表上這段 commits 欄的起點**為準。
兩者在「回頭做 `BLOCKED` 過的段落」時會不一樣 —— 中間插做別段會把 `--segment-base` 往後推,
...
8. **評分 & 修正本輪動過的 CLAUDE.md** — **第一個動作是算範圍,不是開始審**。
   下面這段是**沒有進度表**時的寫法;**有進度表時 `base` 改填表上這段 commits 欄的起點** ——
   步驟 3、4 寫的是 `<起點>` 佔位符、會自動套用前言那條例外,只有這裡把指令寫死,
   照抄就會拿到被往後推的窄範圍(回頭做 `BLOCKED` 過的段落時正是如此):
...
照功能數算,同一個功能的資料、API、畫面算一段;產生帳款、改帳款狀態、金額計算、改權限規則、
批次或排程刪除這類高風險工作,不跟其他功能算同一段。
N ≥ 2 就呼叫 `task-planner`,由它拆段落、出計畫給使用者批准 —— 使用者沒提「拆任務」也一樣。
根目錄 CLAUDE.md 有「進度以…為準」那句時照表接手、不再叫;N 只算表上沒有的新工作。
