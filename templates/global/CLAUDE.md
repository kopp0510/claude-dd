...
**原生 `/goal` 指令**(Claude Code ≥ 2.1.139):上述「迴圈直到驗證通過」可交給原生
`/goal <完成條件>` 自動執行 — 設定完成條件後,Claude 跨多輪自主工作,每輪由獨立評估器
檢查條件,未達成就再跑一輪,直到滿足或 `/goal clear`。

何時用:大型、有明確可驗證收斂終點的工作(如「所有測試通過」「build exit 0」「待辦佇列清空」)。

寫好完成條件:
- 單一可量測終點 — 測試結果、build exit code、檔案數、空佇列
- 明示檢查方式 — 如 `npm test exits 0`、`git status is clean`(評估器只讀 Claude 輸出,
